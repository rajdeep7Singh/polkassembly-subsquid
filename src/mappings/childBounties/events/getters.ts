...
export function getChildBountyAwardedData(itemEvent: any): ChildBountyAwardedData {
    if (awarded.v274.is(itemEvent)) {
        const { index, childIndex, beneficiary } = awarded.v274.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex,
            beneficiary,
...
export function getChildBountyCancelledData(itemEvent: any): ChildBountyCancelledData {
    if (canceled.v274.is(itemEvent)) {
        const { index, childIndex } = canceled.v274.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex
        }
...
export function getChildBountyClaimedData(itemEvent: any): ChildBountyClaimedData {
    if (claimed.v274.is(itemEvent)) {
        const { index, childIndex, payout, beneficiary } = claimed.v274.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex,
            payout,
...
export function getChildBountyAddedData(itemEvent: any): ChildBountyAddedData {
    if (added.v274.is(itemEvent)) {
        const { index, childIndex } = added.v274.decode(itemEvent)
        return {
            parentIndex: index,
            childIndex
        }
