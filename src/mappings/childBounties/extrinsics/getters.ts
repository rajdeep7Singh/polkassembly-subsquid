...
export function getAccepterCuratorData(itemCall: any): AccepterCuratorData {
    if (acceptCurator.v3.is(itemCall)) {
        const { parentBountyId, childBountyId } =acceptCurator.v3.decode(itemCall)
        return {
            childBountyId,
            parentBountyId,
        }
...
export function getUnassingCuratorData(itemCall: any): UnassingCuratorData {
    if (unassignCurator.v3.is(itemCall)) {
        const {parentBountyId, childBountyId} = unassignCurator.v3.decode(itemCall)
        return {
            parentBountyId,
            childBountyId,
        }
...
export function getProposeCuratorData(itemCall: any): ProposeCuratorData {
    if (proposeCurator.v3.is(itemCall)) {
        const { parentBountyId, childBountyId, curator, fee } = proposeCurator.v3.decode(itemCall)
        return {
