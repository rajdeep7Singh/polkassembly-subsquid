...
export function getAccepterCuratorData(itemCall: any): AccepterCuratorData {
    if (acceptCurator.v1.is(itemCall)) {
        const { parentBountyId, childBountyId } =acceptCurator.v1.decode(itemCall)
        return {
            childBountyId,
            parentBountyId,
        }
...
export function getUnassingCuratorData(itemCall: any): UnassingCuratorData {
    if (unassignCurator.v1.is(itemCall)) {
        const {parentBountyId, childBountyId} = unassignCurator.v1.decode(itemCall)
        return {
            parentBountyId,
            childBountyId,
        }
...
export function getProposeCuratorData(itemCall: any): ProposeCuratorData {
    if (proposeCurator.v1.is(itemCall)) {
        const { parentBountyId, childBountyId, curator, fee } = proposeCurator.v1.decode(itemCall)
        return {
