...
export function getAccepterCuratorData(itemCall: any): AccepterCuratorData {
    if (acceptCurator.v274.is(itemCall)) {
        const { parentBountyId, childBountyId } =acceptCurator.v274.decode(itemCall)
        return {
            childBountyId,
            parentBountyId,
        }
...
export function getUnassingCuratorData(itemCall: any): UnassingCuratorData {
    if (unassignCurator.v274.is(itemCall)) {
        const {parentBountyId, childBountyId} = unassignCurator.v274.decode(itemCall)
        return {
            parentBountyId,
            childBountyId,
        }
...
export function getProposeCuratorData(itemCall: any): ProposeCuratorData {
    if (proposeCurator.v274.is(itemCall)) {
        const { parentBountyId, childBountyId, curator, fee } = proposeCurator.v274.decode(itemCall)
        return {
