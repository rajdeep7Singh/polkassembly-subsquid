...
export function getApprovedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): Uint8Array {
    ...
    if (event.isV9300) {
        return event.asV9300.proposalHash
    } else {
...
export function getExecutedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): Uint8Array {
    ...
    if (event.isV9300) {
        ...
        return event.asV9310.proposalHash
    } else if (event.isV9420) {
        return event.asV9420.proposalHash
    } else if (event.isV9430) {
        return event.asV9430.proposalHash
    } else {
...
export function getProposedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): ProposedData {
    ...
    if (event.isV9300) {
        const { account, proposalIndex, proposalHash, threshold } = event.asV9300
        return {
            proposer: account,
            index: proposalIndex,
            hash: proposalHash,
...
export function getVotedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): VotedData {
    ...
    if (event.isV9300) {
        const { account, proposalHash, voted } = event.asV9300
        return {
            voter: account,
            hash: proposalHash,
            decision: voted,
