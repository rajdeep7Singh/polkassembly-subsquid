...
export function getApprovedData(itemEvent: Event): string {
    if (approved.v10005.is(itemEvent)) {
        return approved.v10005.decode(itemEvent).proposalHash
    } else {
...
export function getClosedData(itemEvent: Event): string {
    if (closed.v10005.is(itemEvent)) {
        return closed.v10005.decode(itemEvent).proposalHash
    } else {
        throw new UnknownVersionError(itemEvent.name)
    }
}
...
export function getDisapprovedData(itemEvent: Event): string {
    if (disapproved.v10005.is(itemEvent)) {
        return disapproved.v10005.decode(itemEvent).proposalHash
    } else {
        throw new UnknownVersionError(itemEvent.name)
    }
}
...
export function getExecutedData(itemEvent: Event): string {
    if (executed.v10005.is(itemEvent)) {
        return executed.v10005.decode(itemEvent).proposalHash
...
export function getProposedData(itemEvent: Event): ProposedData {
    if (proposed.v10005.is(itemEvent)) {
        const { account, proposalIndex, proposalHash, threshold } = proposed.v10005.decode(itemEvent)
        return {
            proposer: account,
            index: proposalIndex,
            hash: proposalHash,
...
export function getVotedData(itemEvent: Event): VotedData {
    if (voted.v10005.is(itemEvent)) {
        const { account, proposalHash, voted: voteData } = voted.v10005.decode(itemEvent)
        return {
            voter: account,
            hash: proposalHash,
            decision: voteData,
