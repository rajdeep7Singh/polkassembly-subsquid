...
export function getApprovedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): Uint8Array {
    ...
    if (event.isV5) {
        ...
        return event.asV8.proposalHash
    } else {
...
export function getExecutedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): Uint8Array {
    ...
    if (event.isV5) {
        ...
        return event.asV13.proposalHash
    }else if (event.isV20) {
        return event.asV20.proposalHash
    }  else {
...
export function getProposedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): ProposedData {
    ...
    if (event.isV5) {
        const [proposer, index, hash, threshold] = event.asV5
        return {
            proposer,
            index,
            hash,
        ...
        const { account, proposalIndex, proposalHash, threshold } = event.asV8
        return {
            proposer: account,
            index: proposalIndex,
            hash: proposalHash,
...
export function getVotedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): VotedData {
    ...
    if (event.isV5) {
        const [voter, hash, decision] = event.asV5
        return {
            voter,
            hash,
            decision,
        ...
        const { account, proposalHash, voted } = event.asV8
        return {
            voter: account,
            hash: proposalHash,
            decision: voted,
