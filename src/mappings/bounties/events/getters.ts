...
export function getBountyAwardedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyAwardedData {
    ...
    if (event.isV268) {
        const [index, beneficiary] = event.asV268
        return {
            index,
            beneficiary,
        }
    } else if (event.isV274) {
        const { index, beneficiary } = event.asV274
        return {
            index,
            beneficiary,
        }
...
export function getBountyBacameActiveData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyBacameActiveData {
    ...
    if (event.isV268) {
        const index = event.asV268
        return {
            index,
        }
    } else if (event.isV274) {
        const { index } = event.asV274
        return {
            index,
        }
    } else {
...
export function getBountyClaimedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyClaimedData {
    ...
    if (event.isV268) {
        const [index, payout, beneficiary] = event.asV268
        return {
            index,
            payout,
            beneficiary,
        ...
        const { index, payout, beneficiary } = event.asV274
        return {
            index,
            payout,
            beneficiary,
