...
export function getBountyAwardedDataOld(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyAwardedData {
    ...
    if (event.isV266) {
        const [index, beneficiary] = event.asV266
        return {
            index,
            beneficiary,
        }
...
export function getBountyAwardedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyAwardedData {
    ...
    if (event.isV283) {
        const [index, beneficiary] = event.asV283
        return {
            index,
            beneficiary,
        }
    } else if (event.isV297) {
        const { index, beneficiary } = event.asV297
        return {
            index,
            beneficiary,
        }
...
export function getBountyBacameActiveData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyBacameActiveData {
    ...
    if (event.isV283) {
        const index = event.asV283
        return {
            index,
        }
    } else if (event.isV297) {
        const { index } = event.asV297
        return {
            index,
        }
    } else {
...
export function getBountyClaimedDataOld(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyClaimedData {
    ...
    if (event.isV266) {
        const [index, payout, beneficiary] = event.asV266
        return {
            index,
            beneficiary,
            payout,
...
export function getBountyClaimedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): BountyClaimedData {
    ...
    if (event.isV283) {
        const [index, payout, beneficiary] = event.asV283
        return {
            index,
            payout,
            beneficiary,
        ...
        const { index, payout, beneficiary } = event.asV297
        return {
            index,
            payout,
            beneficiary,
