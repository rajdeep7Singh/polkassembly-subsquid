...
export function getProposedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): ProposedData {
    ...
    if (event.isV200) {
        const index = event.asV200
        return {
            index,
        }
    } else if (event.isV1000) {
        const { proposalIndex: index } = event.asV1000
        return {
            index,
        }
    }  else {
...
export function getAwarderData(ctx: BatchContext<Store, unknown>, itemEvent: Event): AwarderData {
    ...
    if (event.isV200) {
        const [index, deposit, beneficiary] = event.asV200
        return {
            index,
        }
    } 
...
export function getSpendApprovedData(ctx: BatchContext<Store, unknown>, itemEvent: Event): SpendApprovedData {
    ...
    if (event.isV10002) {
        const { proposalIndex, amount, beneficiary}= event.asV10002
        return {
            proposalIndex,
            amount,
            beneficiary
