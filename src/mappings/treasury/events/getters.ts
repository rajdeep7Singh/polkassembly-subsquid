...
export function getAwarderData(itemEvent: Event): AwarderData {
    if (awarded.v5000.is(itemEvent)) {
        const { proposalIndex: index } = awarded.v5000.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getSpendApprovedData(itemEvent: Event): SpendApprovedData {
    if (spendApproved.v5000.is(itemEvent)) {
        const { proposalIndex, amount, beneficiary}= spendApproved.v5000.decode(itemEvent)
        return {
            proposalIndex,
            amount,
            beneficiary
