...
export function getProposedData(itemEvent: Event): ProposedData {
    if (proposed.v1.is(itemEvent)) {
        const { proposalIndex: index } = proposed.v1.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getRejectedData(itemEvent: Event): RejectedData {
    if (rejected.v1.is(itemEvent)) {
        const { proposalIndex: index } = rejected.v1.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getAwarderData(itemEvent: Event): AwarderData {
    if (awarded.v1.is(itemEvent)) {
        const { proposalIndex: index } = awarded.v1.decode(itemEvent)
        return {
            index,
        }
    } else {
...
export function getSpendApprovedData(itemEvent: Event): SpendApprovedData {
    if (spendApproved.v1.is(itemEvent)) {
        const { proposalIndex, amount, beneficiary}= spendApproved.v1.decode(itemEvent)
        return {
            proposalIndex,
            amount,
            beneficiary
