...
export function getEventData(itemEvent: Event): ReferendumEventData {
    if (submitted.v10038.is(itemEvent)) {
        const {index, track, proposal } = submitted.v10038.decode(itemEvent)
        let hash = null;
...
export function getCancelledData(itemEvent: Event): ReferendaData {
    if (cancelled.v10038.is(itemEvent)) {
        const { index, tally } = cancelled.v10038.decode(itemEvent)
        return {
            index,
            tally
        }
...
export function getApprovedData(itemEvent: Event): ReferendaIndexData {
    if (approved.v10038.is(itemEvent)) {
        const { index } = approved.v10038.decode(itemEvent)
        return {
            index
        }
    } else {
...
export function getKilledData(itemEvent: Event): ReferendaData {
    if (killed.v10038.is(itemEvent)) {
        const { index, tally } = killed.v10038.decode(itemEvent)
        return {
            index,
            tally
        }
...
export function getTimedOutData(itemEvent: Event): ReferendaData {
    if (timedOut.v10038.is(itemEvent)) {
        const { index, tally } = timedOut.v10038.decode(itemEvent)
        return {
            index,
            tally
        }
...
export function getRejectedData(itemEvent: Event): ReferendaData {
    if (rejected.v10038.is(itemEvent)) {
        const { index, tally } = rejected.v10038.decode(itemEvent)
        return {
            index,
            tally
        }
...
export function getConfirmAbortedData(itemEvent: Event): ReferendaIndexData {
    if (confirmAborted.v10038.is(itemEvent)) {
        const { index } = confirmAborted.v10038.decode(itemEvent)
        return {
            index
        }
    } else {
...
export function getConfirmedData(itemEvent: Event): ReferendaData {
    if (confirmed.v10038.is(itemEvent)) {
        const { index, tally } = confirmed.v10038.decode(itemEvent)
        return {
            index,
            tally
        }
...
export function getConfirmStartedData(itemEvent: Event): ReferendaIndexData {
    if (confirmStarted.v10038.is(itemEvent)) {
        const { index } = confirmStarted.v10038.decode(itemEvent)
        return {
            index
        }
    } else {
...
export function getDecisionDepositPlacedData(itemEvent: Event): ReferendaDepositData {
    if (decisionDepositPlaced.v10038.is(itemEvent)) {
        const { index, who, amount } = decisionDepositPlaced.v10038.decode(itemEvent)
        return {
            index,
            who,
            amount
...
export function getDecisionStartedData(itemEvent: Event): ReferendaDecisionStartedData {
    if (decisionStarted.v10038.is(itemEvent)) {
        let hash = undefined;
        const { index, track, proposal, tally} = decisionStarted.v10038.decode(itemEvent)
        if(proposal.__kind == "Inline") {
            hash = proposal.value
        }
        else{
...
export function getMetadataSetData(itemEvent: Event): ReferendaMetadataSetData {
    if (metadataSet.v10038.is(itemEvent)) {
        const { index, hash } = metadataSet.v10038.decode(itemEvent)
        return {
            index,
            hash
        }
...
export function getMetadataClearedData(itemEvent: Event): ReferendaMetadataCleared {
    if (metadataCleared.v10038.is(itemEvent)) {
        const { index, hash } = metadataCleared.v10038.decode(itemEvent)
        return {
            index,
            hash
        }
