...
export function getPreimageNotedData(itemEvent: Event): PreimageNotedData {
    if (noted.v10500.is(itemEvent)) {
        const { hash } = noted.v10500.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageRequestedData(itemEvent: Event): PreimageRequestedData {
    if (requested.v10500.is(itemEvent)) {
        const {hash} = requested.v10500.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageClearedData(itemEvent: Event): PreimageRequestedData {
    if (cleared.v10500.is(itemEvent)) {
        const {hash} = cleared.v10500.decode(itemEvent)
        return {
            hash
        }
    } else {
