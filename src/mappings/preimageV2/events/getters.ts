...
export function getPreimageNotedData(itemEvent: Event): PreimageNotedData {
    if (noted.v100.is(itemEvent)) {
        const { hash } = noted.v100.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageRequestedData(itemEvent: Event): PreimageRequestedData {
    if (requested.v100.is(itemEvent)) {
        const {hash} = requested.v100.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageClearedData(itemEvent: Event): PreimageRequestedData {
    if (cleared.v100.is(itemEvent)) {
        const {hash} = cleared.v100.decode(itemEvent)
        return {
            hash
        }
    } else {
