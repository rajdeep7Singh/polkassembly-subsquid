...
export function getPreimageNotedData(itemEvent: Event): PreimageNotedData {
    if (noted.v1000.is(itemEvent)) {
        const { hash } = noted.v1000.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageRequestedData(itemEvent: Event): PreimageRequestedData {
    if (requested.v1000.is(itemEvent)) {
        const {hash} = requested.v1000.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageClearedData(itemEvent: Event): PreimageRequestedData {
    if (cleared.v1000.is(itemEvent)) {
        const {hash} = cleared.v1000.decode(itemEvent)
        return {
            hash
        }
    } else {
