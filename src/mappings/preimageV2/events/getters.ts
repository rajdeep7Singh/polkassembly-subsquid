...
export function getPreimageNotedData(itemEvent: Event): PreimageNotedData {
    if (noted.v9300.is(itemEvent)) {
        const { hash } = noted.v9300.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageRequestedData(itemEvent: Event): PreimageRequestedData {
    if (requested.v9300.is(itemEvent)) {
        const {hash} = requested.v9300.decode(itemEvent)
        return {
            hash
        }
    } else {
...
export function getPreimageClearedData(itemEvent: Event): PreimageRequestedData {
    if (cleared.v9300.is(itemEvent)) {
        const {hash} = cleared.v9300.decode(itemEvent)
        return {
            hash
        }
    } else {
