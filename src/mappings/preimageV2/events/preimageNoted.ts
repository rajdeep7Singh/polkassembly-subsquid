...
async function getStorageData(ctx: BatchContext<Store, unknown>, hash: Uint8Array, block: SubstrateBlock): Promise<PreimageStorageData | undefined> {
    ...
    if (storage.isV274) {
        const storageData = await storage.asV274.get(hash)
        ...
        if (!preimageStatus || !preimageStatus.len) return undefined
        const storageData = await storage.asV283.get([hash, preimageStatus?.len])
        if (!storageData) return undefined
...
export async function getPreimageStatusData(ctx: BatchContext<Store, unknown>, hash: Uint8Array, block: SubstrateBlock): Promise<PreimageStatusStorageData | undefined> {
    ...
    if (preimageStorage.isV274) {
        const storageData = await preimageStorage.asV274.get(hash)
        if (!storageData) return undefined
        return {
    ...
    }else if (preimageStorage.isV283) {
        const storageData = await preimageStorage.asV283.get(hash)
        if (!storageData) return undefined
        return {
            status: storageData.__kind,
