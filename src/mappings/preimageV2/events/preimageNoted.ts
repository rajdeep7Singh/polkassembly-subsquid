...
async function getStorageData(ctx: BatchContext<Store, unknown>, hash: Uint8Array, block: SubstrateBlock): Promise<PreimageStorageData | undefined> {
    ...
    if (storage.isV10500) {
        const storageData = await storage.asV10500.get(hash)
        ...
        if(preimageStatus && preimageStatus.len){
            const storageData = await storage.asV10890.get([hash, preimageStatus.len])
            if (!storageData) return undefined
            return {
                data: storageData,
                ...preimageStatus
...
export async function getPreimageStatusData(ctx: BatchContext<Store, unknown>, hash: Uint8Array, block: SubstrateBlock): Promise<PreimageStatusStorageData | undefined> {
    ...
    if (preimageStorage.isV10500) {
        const storageData = await preimageStorage.asV10500.get(hash)
        if (!storageData) return undefined
        return {
    ...
    } if (preimageStorage.isV10890) {
        const storageData = await preimageStorage.asV10890.get(hash)
        if (!storageData) return undefined
        return {
            status: storageData.__kind,
