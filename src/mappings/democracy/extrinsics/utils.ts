...
export async function addDelegatedVotesReferendum(ctx: ProcessorContext<Store>, block: number, blockTime: number, nestedDelegations: VotingDelegation[], convictionVote: ConvictionVote): Promise<{delegatedVotes: ConvictionDelegatedVotes[], delegatedVotePower: bigint, flattenedVotes: FlattenedConvictionVotes[]}> {
...
export async function handleSubstrateAndPrecompileVotes(ctx: ProcessorContext<Store>, header: any, index: number, vote: any, from: any, isSubstrate: boolean, extrinsicIndex?: string, txHash?: string): Promise<void>{
    ...
    if (!proposal) {
        ctx.log.warn(MissingProposalRecordWarn(ProposalType.Referendum, index, extrinsicIndex))
        return
    }
...
export async function handleSubtrateAndPrecompileDelegationVote(ctx: ProcessorContext<Store>, header: any, toWallet: string, lockPeriod: number, balance: bigint, from: string, extrinsicIndex?: string): Promise<void> {
...
export async function handleSubstratAndPrecompileRemoveVote(ctx: ProcessorContext<Store>, header: any, index: number, wallet: string, extrinsicIndex?: string, txnHash?: string): Promise<void> {
    ...
    await removeVote(ctx, wallet, index, header.height, header.timestamp, true, extrinsicIndex, txnHash)    
}

export async function handleSubstrateAndPrecompileUndelegate(ctx: ProcessorContext<Store>, header: any, from: string, extrinsicIndex?: string): Promise<void> {
    let delegation = null;
    const delegations = await ctx.store.find(VotingDelegation, { where: { from, endedAtBlock: IsNull(), type: DelegationType.Democracy } })
