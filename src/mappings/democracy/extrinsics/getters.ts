...
export function getVoteData(itemCall: any): DemocracyVoteCallData {
    if (vote.v200.is(itemCall)) {
        const { refIndex, vote: voteData } = vote.v200.decode(itemCall)
...
export function getDelegateData(itemCall: any): ConvictionVoteDelegateCallData {
    if (delegate.v200.is(itemCall)) {
        const { to, conviction, balance } = delegate.v200.decode(itemCall)
        return {
            to: to,
            lockPeriod: convictionToLockPeriod(conviction.__kind),
            balance
        }
    } else if (delegate.v10009.is(itemCall)) {
        const { to, conviction, balance } = delegate.v10009.decode(itemCall)
        return {
...
export function getRemoveVoteData(itemCall: any): ConvictionVotingRemoveVoteCallData {
    if (removeVote.v200.is(itemCall)) {
        const eventData = removeVote.v200.decode(itemCall)
        return {
...
export function getRemoveOtherVoteData(itemCall: any): ConvictionVotingRemoveOtherVoteCallData {
    if (removeOtherVote.v200.is(itemCall)) {
        const { target, index } = removeOtherVote.v200.decode(itemCall)
        return {
            target,
            index
        }
    } else if (removeOtherVote.v10009.is(itemCall)) {
        const { target, index } = removeOtherVote.v10009.decode(itemCall)
        return {
