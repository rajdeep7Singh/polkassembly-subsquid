...
export function getVoteData(itemCall: any): DemocracyVoteCallData {
    if (vote.v2100.is(itemCall)) {
        const { refIndex, vote: voteData } = vote.v2100.decode(itemCall)
...
export function getDelegateData(itemCall: any): ConvictionVoteDelegateCallData {
    if (delegate.v2100.is(itemCall)) {
        const { to, conviction, balance } = delegate.v2100.decode(itemCall)
        return {
            to: to,
...
export function getRemoveOtherVoteData(itemCall: any): ConvictionVotingRemoveOtherVoteCallData {
    if (removeOtherVote.v2100.is(itemCall)) {
        const { target, index } = removeOtherVote.v2100.decode(itemCall)
        return {
            target,
            index
        }
