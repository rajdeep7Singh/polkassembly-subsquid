...
export class ChildBountiesAcceptCuratorCall {
     ...
     * - `child_bounty_id`: Index of child bounty.
     */
    get asV274(): {parentBountyId: number, childBountyId: number} {
        assert(this.isV274)
        return this._chain.decodeCall(this.call)
    }
}
...
export class DemocracyDelegateCall {
     ...
     *    voted on. Weight is charged as if maximum votes.
     */
    get asV273(): {to: Uint8Array, conviction: v273.Conviction, balance: bigint} {
        assert(this.isV273)
        return this._chain.decodeCall(this.call)
    }
...
export class DemocracyVoteCall {
     ...
     *  Weight: `O(R)` where R is the number of referendums the voter has voted on.
     */
    get asV273(): {refIndex: number, vote: v273.AccountVote} {
        assert(this.isV273)
        return this._chain.decodeCall(this.call)
    }
     ...
     * Weight: `O(R)` where R is the number of referendums the voter has voted on.
     */
    get asV274(): {refIndex: number, vote: v274.AccountVote} {
        assert(this.isV274)
        return this._chain.decodeCall(this.call)
    }
}
