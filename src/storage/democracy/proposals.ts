...
async function getStorageData(ctx: ProcessorContext<Store>, block: any): Promise<DemocracyProposalStorageData[] | undefined> {
    if (publicProps.v200.is(block)) {
        const storageData = await publicProps.v200.get(block)
    ...
    } else if(publicProps.v10009.is(block)){
        const storageData = await publicProps.v10009.get(block)
        if (!storageData) return undefined
