...
async function getStorageData(ctx: ProcessorContext<Store>, block: any): Promise<DemocracyProposalStorageData[] | undefined> {
    if (publicProps.v9300.is(block)) {
        const storageData = await publicProps.v9300.get(block)
    ...
    } else if(publicProps.v9310.is(block)){
        const storageData = await publicProps.v9310.get(block)
        if (!storageData) return undefined
