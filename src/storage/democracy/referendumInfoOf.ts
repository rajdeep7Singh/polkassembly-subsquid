...
import * as v172 from '../../types/v172'
import * as v194 from '../../types/v194'
...
async function getStorageData(ctx: BatchContext<Store, unknown>, index: number, block: SubstrateBlock): Promise<ReferendumStorageData | undefined> {
    ...
    if (storage.isV172) {
        const storageData = await storage.asV172.get(index)
        if (!storageData) return undefined

        const { __kind: status } = storageData
        if (status === 'Ongoing') {
            const { proposalHash: hash, end, delay, threshold } = (storageData as v172.ReferendumInfo_Ongoing).value
            return {
                status,
                hash,
                end,
            ...
            const { end, approved } = storageData as v172.ReferendumInfo_Finished
            return {
                status,
                end,
                approved,
        ...
        const storageData = await storage.asV194.get(index)
        if (!storageData) return undefined

        const { __kind: status } = storageData
        if (status === 'Ongoing') {
            let hash
            const { proposal, end, delay, threshold } = (storageData as v194.ReferendumInfo_Ongoing).value
            ...
            const { end, approved } = storageData as v194.ReferendumInfo_Finished
            return {
                status,
                end,
                approved,
