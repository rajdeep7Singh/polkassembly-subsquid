...
import * as v200 from '../../types/v200'
...
async function getStorageData(ctx: ProcessorContext<Store>, index: number, block: any): Promise<ReferendumStorageData | undefined> {
    if (referendumInfoOf.v200.is(block)) {
        const storageData = await referendumInfoOf.v200.get(block, index)
        if (!storageData) return undefined

        const { __kind: status } = storageData
        if (status === 'Ongoing') {
            const { proposalHash: hash, end, delay, threshold } = (storageData as v200.ReferendumInfo_Ongoing).value
            return {
                status,
                hash,
                end,
            ...
            const { end, approved } = storageData as v200.ReferendumInfo_Finished
            return {
                status,
                end,
                approved,
    ...
    else if(referendumInfoOf.v10009.is(block)){
        const storageData = await referendumInfoOf.v10009.get(block, index)
        if (!storageData) return undefined
