...
import * as v5000 from '../../types/v5000'
...
async function getStorageData(ctx: ProcessorContext<Store>, index: number, block: any): Promise<ReferendumStorageData | undefined> {
    if(referendumInfoOf.v5000.is(block)){
        const storageData = await referendumInfoOf.v5000.get(block, index)
        if (!storageData) return undefined

        const { __kind: status } = storageData
        if (status === 'Ongoing') {
            let hash
            const { proposal, end, delay, threshold } = (storageData as v5000.ReferendumInfo_Ongoing).value
            ...
            const { end, approved } = storageData as v5000.ReferendumInfo_Finished
            return {
                status,
                end,
                approved,
